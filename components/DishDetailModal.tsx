...
export const DishDetailModal: React.FC<DishDetailModalProps> = ({ 
  ...
  return (
    <div 
      className="fixed inset-0 z-[300] bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-200 overflow-y-auto flex flex-col items-center justify-start sm:justify-center p-0"
      onClick={onClose}
    >
      {/* 手机端悬浮关闭按钮 */}
      <button 
      ...
      {/* 核心改动：整体作为一个容器随外层滚动 */}
      <div 
        className="bg-[#fcfbf9] w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[3rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 duration-300 relative flex flex-col my-0 sm:my-8"
            ...
            <div className="grid grid-cols-2 gap-4">
              {isLoadingDetail ? (
                ...
                <button 
                  key={i} 
                  onClick={(e) => {
                    e.stopPropagation(); // 关键：防止点击食材关闭 Modal
                    onIngredientClick({
