...
export const DishDetailModal: React.FC<DishDetailModalProps> = ({ dish, onClose, onIngredientClick, onSpicyClick }) => {
  ...
  const spicyLevel = dish.spiciness || 0;
      ...
      >
        <div className="relative bg-red-600 pt-10 pb-12 px-8 text-white border-b-4 border-yellow-400">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={speakDishName} className="bg-white/20 hover:bg-white/30 p-2 rounded-xl transition-colors active:scale-90">
                <SpeakerIcon className="w-6 h-6" />
              </button>
            </div>
            {(dish.pinyin || dish.pronunciation_guide) && (
              <div className="mt-3 bg-white/10 rounded-xl p-3 inline-block">
                {dish.pinyin && <p className="text-sm font-medium tracking-wide text-white/90 italic">{dish.pinyin}</p>}
                {dish.pronunciation_guide && <p className="text-[10px] font-semibold uppercase tracking-widest text-yellow-300 mt-0.5">Sounds like: {dish.pronunciation_guide}</p>}
              </div>
            )}
          </div>
          ...
          <div className="absolute -bottom-6 left-8 right-8 flex justify-between items-center">
             ...
             <button onClick={handleImageSearch} className="bg-white text-slate-900 hover:bg-slate-100 px-4 py-2 rounded-2xl font-medium text-sm shadow-xl flex items-center gap-2 transition-all active:scale-95">🔍 Search Photos</button>
          </div>
        </div>
        ...
        <div className="p-8 pt-10 overflow-y-auto flex-1 space-y-8 text-slate-900 text-left">
          {dish.has_animal_fats && (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex gap-3 animate-pulse">
              ...
              <div>
                ...
                <p className="text-amber-800 text-xs font-medium leading-normal mt-1">This dish is often cooked with <strong>Lard (猪油)</strong> or <strong>Tallow (牛油)</strong>.</p>
              </div>
            </div>
          )}
          ...
          <section>
            ...
            <p className="text-slate-700 leading-relaxed text-lg font-medium">{dish.description}</p>
          </section>
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-3">
              {(dish.classic_ingredients || []).map((ing: any, i: number) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button onClick={onSpicyClick} className="bg-slate-50 p-5 rounded-[2rem] border-2 border-slate-100 text-left hover:bg-red-50 hover:border-red-200 transition-all group relative">
              <h4 className="text-[10px] font-semibold text-slate-400 uppercase mb-3 group-hover:text-red-500">Spice Meter</h4>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <ChiliIcon key={i} className={`w-5 h-5 transition-transform duration-300 ${i < spicyLevel ? 'text-red-600 scale-110' : 'text-slate-200'}`} filled={i < spicyLevel} />
                    ))}
                  </div>
                  <span className="text-[10px] font-bold text-red-600 uppercase tracking-tighter">{spicyInfo.label}</span>
                </div>
                <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-yellow-400 via-orange-500 to-red-600" style={{ width: `${(spicyLevel / 5) * 100}%` }}></div>
                </div>
              </div>
            </button>
            ...
            <div className="bg-slate-50 p-5 rounded-[2rem] border-2 border-slate-100 flex flex-col justify-center">
              ...
              <div className="flex flex-wrap gap-1.5">
                {dish.allergens?.length > 0 ? (
                  ...
                  <span className="text-green-700 font-medium text-xs flex items-center gap-1">None Detected</span>
                )}
              </div>
            </div>
          </div>
