...
// 将主逻辑封装在 MainLayout 中，以便在 Router 内部使用 hook
const MainLayout: React.FC = () => {
  ...
  const onPurchaseSuccess = (newUserData: any) => {
    ...
    const msg = isPremium ? t('common.purchaseSuccessPremium') : t('common.purchaseSuccessCredits');
    ...
    try {
      ...
      if (mode === RecognitionMode.MENU) {
        ...
        if (rawResult) {
          ...
          const finalStoreData = (rawResult as any).store || (rawResult.store_name ? rawResult : null) || {
            store_name: t('storeCard.localShop'),
            description: t('storeCard.defaultDescription'),
            cuisine_type: "", specialty_dishes: [], average_price_range: ""
          };
    ...
    <div className="min-h-screen pb-0 bg-[#fafafa] font-sans w-full overflow-x-hidden">
          ...
          <Route path="/" element={
            <>
              ...
              {status === AppStatus.IDLE && (
                <HomeIdleView
                  mode={mode} onModeChange={handleModeChange} onTriggerUpload={triggerUpload}
                  onOpenSurvival={() => setShowSurvival(true)} onPurchase={() => setShowPricing(true)}
                  onHandleDailyShare={() => handleDailyShare(getOrCreateUserId())}
                  usage={usage} onShowDishDetail={handleDishClick}
                  onGameWin={() => handleGameWin(getOrCreateUserId())}
                  showGame={showGame} onOpenGame={() => setShowGame(true)} onCloseGame={() => setShowGame(false)}
                />
              )}
      ...
      {showIntroOverlay && (
        <div className="fixed inset-0 z-[11000] bg-slate-900/90 backdrop-blur-xl flex flex-col items-center justify-center p-8 animate-in fade-in duration-300">
          <button onClick={() => { setShowIntroOverlay(false); triggerUpload(); }} className="group flex flex-col items-center gap-6 active:scale-95 transition-all">
      ...
      {creditUpdateMessage && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[10000] w-full max-w-xs px-4 pointer-events-none">
          <div className="pointer-events-auto"><CreditUpdateCard message={creditUpdateMessage} onClose={() => setCreditUpdateMessage(null)} /></div>
        </div>
      )}
    </div>
  );
