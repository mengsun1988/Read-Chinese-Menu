...
let userData = { credits: 200, scanCount: 0, lastUsed: new Date().toISOString(), passExpiryDate: null };
      ...
      if (type === "dish_detail") {
        qwenPayload = {
          ...
          messages: [{
            role: "user",
            content: `CRITICAL: Analyze the dish "${name_cn}". Capture ALL text including faint descriptions or small ingredient lists. Return JSON: { "ingredients": [{"name_cn": "...", "name_en": "..."}], "spiciness_level": 0-5, "pinyin": "mandarin pinyin with tones", "pronunciation": "English phonetic guide", "allergens": ["..."], "description": "Full translation of the dish's secondary/small text or a brief authentic description", "has_animal_fats": true/false }. JSON ONLY.`
          }],
        ...
        qwenPayload = {
          ...
          messages: [{
            role: "user",
            content: [{ type: "text", text: "Identify this shop or restaurant. Return JSON: { name_cn, name_en, type, confidence, description }." }, { type: "image_url", image_url: { url: `data:image/jpeg;base64,${base64Image}` } }]
          }],
          response_format: { type: "json_object" }
        };
      } else {
...
if (userData.scanCount === 4) { userData.credits += 50; achievementTriggered = "milestone_4"; }
      ...
      let responseBody = type === "dish_detail" 
        ? { ...parsedData, isFullyAnalyzed: true }
        : type === "street"
        ? parsedData
        : {
