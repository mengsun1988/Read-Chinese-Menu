  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'POST' && isScanPath) {
      try {
        ...
        // 核心修正：如果是详情模式，不强制要求 image 字段
        if (!userId || (type !== "dish_detail" && !base64Image)) {
        ...
        const getDetailPrompt = () => `Analyze dish "${name_cn}" and return JSON. Target language: ${lang}.
          CRITICAL: Identify hidden allergens. 
          ...
          Ingredients: Return list where each MUST have "name_cn" and "name_translated".
          Return JSON: { 
            "name_translated": "...", 
            "classic_ingredients": [{"name_cn": "...", "name_translated": "..."}], 
            "spiciness_level": 0-5, "pinyin": "Standard Pinyin", 
...
const getStorePrompt = () => `Identify restaurant storefront and return JSON. 
        ...
        const taskGemini = async () => {
          if (type === "dish_detail" && !base64Image) throw new Error("Gemini requires image for this route");
    ...
    // --- API: Payment Verification (Paypal 校验保留) ---
